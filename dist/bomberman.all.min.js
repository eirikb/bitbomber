...
Powerup=function(){OGE.Body.apply(this,arguments)};Powerup.prototype=Object.construct_prototype(OGE.Body);Game=function(c,f){this.world=new OGE.World(c,f,16);this.players=[];this.bombs=[];this.fires=[];this.blocks=[];this.bricks=[];this.maxPlayers=4};
Game.prototype.addBody=function(c,f){if(!this.world.addBody(c,f))return false;if(c instanceof Player){if(this.players.length==this.maxPlayers)return false;else if(this.players.length===0)this.owner=c;this.players.push(c)}else c instanceof Bomb&&this.bombs.push(c);return true};
