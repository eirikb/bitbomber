...
OGE.World.prototype.moveBody=function(b,e,i){var h,k,j;if(arguments.length===1){e=b.direction;i=b.speed}for(var n=0;n<i;n++){h=b.x;k=b.y;this.removeBodyFromZones(b);b.x+=e.cos;b.y+=e.sin;if(b.x>=0&&b.y>=0&&b.x+b.width<=this.width&&b.y+b.height<=this.height){j=this.getBodies(b);for(var p=0;p<j.length;p++){var o=j[p];if(b!==o&&!o.intersects(h,k,b.width,b.height)&&o.intersects(b)){var q=b.collide(o)===true;o=o.collide(b)===true;if(q&&o){b.x=h;b.y=k;this.addBodyToZones(b);if(b.slide&&p>=0)this.slideBody(b,
e);else return}}}this.addBodyToZones(b)}else{b.x=h;b.y=k;break}}};
...
Game.prototype.explodeBomb=function(b,e){if(arguments.length===1)e={x:b.x,y:b.y,bodies:[],fires:[],bombs:[]};var i=this,h=function(j,n,p){var o=function(q){return q.x===j&&q.y===n};if(typeof _.detect(e.bodies,o)==="undefined"){p={x:j,y:n,firevar:p};o=_.detect(e.fires,o);if(typeof o!=="undefined"){if(o.firevar!=="c"&&_.contains("c","v","h",p.firevar)){e.fires=_.without(e.fires,o);e.fires.push(p)}}else e.fires.push(p)}};e.bombs.push(b);h(b.x,b.y,"c");var k=function(j,n,p,o){j=j*b.width;var q=b.x+j,
s=b.x+b.size*j;n=n*b.height;var t=b.y+n,u=b.y+b.size*n;actualCheck=function(w,y,C){if(w>=0&&y>=0&&w<i.world.width&&y<i.world.height){for(var z=i.world.getBodies(w,y,b.width,b.height),B=0;B<z.length;B++){var x=z[B];if(x!==b&&x.intersects(w+2,y+2,b.width-2,b.height-2)){if(x.armor>b.power)return false;else{!(x instanceof Bomb)&&!_.contains(e.bodies,x)&&e.bodies.push(x);if(x.armor===b.power)return false}x instanceof Bomb&&!_.contains(e.bombs,x)&&i.explodeBomb(x,e)}}h(w,y,C)}return true};for(q=q;q!==s+
j;q+=j)if(!actualCheck(q,b.y,q!==s?p:o))break;for(j=t;j!==u+n;j+=n)if(!actualCheck(b.x,j,j!==u?p:o))break};k(-1,0,"h","l");k(1,0,"h","r");k(0,-1,"v","u");k(0,1,"v","d");return e};Game.prototype.addBody=function(b,e){if(!this.world.addBody(b,e))return false;if(b instanceof Player){if(this.players.length==this.maxPlayers)return false;else if(this.players.length===0)this.owner=b;this.players.push(b)}else b instanceof Bomb&&this.bombs.push(b);return true};
Game.prototype.serialize=function(){var b=this,e={width:this.world.width,height:this.world.height,guid:this.guid},i=function(h,k){if(b[h].length>0){var j=b[h][0].width;e[h]={};e[h].size=j;e[h].pos=[];e[h].attrs={};_.each(b[h],function(n){e[h].pos.push(n.y/j*Math.floor(b.world.width/j)+n.x/j);_.each(k,function(p){e[h].attrs[p]=n[p]})})}};if(this.players.length>0){e.players=[];_.each(this.players,function(h){e.players.push(h.serialize())})}i("blocks",["armor"]);i("bricks");return e};
