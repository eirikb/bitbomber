...
OGE.World.prototype.getBodies=function(a,b,c,d){var e,f;e=f=a;if(arguments.length===1){f=e.x;b=e.y;c=e.width;d=e.height}f=f<0?0:f;f=f+c>this.width?this.width-c:f;b=b<0?0:b;b=b+d>this.height?this.height-d:b;e=[];f=this.getZones(f,b,c,d);for(var g=0;g<f.length;g++)for(var h=f[g].bodies,i=0;i<h.length;i++){for(var j=h[i],l=false,k=0;k<e.length;k++)if(e[k]===j){l=true;break}l||e.push(j)}return e};
OGE.World.prototype.getZones=function(a,b,c,d){var e,f;e=f=a;if(arguments.length===1){f=e.x;b=e.y;c=e.width;d=e.height}if(f>=0&&f+c-1<this.width&&b>=0&&b+d-1<this.height){e=(f+c)/this.zoneSize<<0;var g=b/this.zoneSize<<0,h=(b+d)/this.zoneSize<<0,i=0,j=[];for(f=f/this.zoneSize<<0;f<=e;f++)for(b=g;b<=h;b++)j[i++]=this.zones[f][b];return j}else return[]};
...
OGE.World.prototype.moveBody=function(a,b,c){var d,e,f;if(arguments.length===1){b=a.direction;c=a.speed}for(var g=0;g<c;g++){d=a.x;e=a.y;this.removeBodyFromZones(a);a.x+=b.cos;a.y+=b.sin;f=this.getBodies(a);for(var h=0;h<f.length;h++){var i=f[h];if(a!==i&&!i.intersects(d,e,a.width,a.height)&&i.intersects(a)){var j=a.collide(i)===true;i=i.collide(a)===true;if(j&&i){a.x=d;a.y=e;this.addBodyToZones(a);if(a.slide&&h>=0)this.slideBody(a,b);else return}}}this.addBodyToZones(a)}};
OGE.World.prototype.slideBody=function(a,b){var c=this,d=function(f){var g=0,h=a.x+f.cos*1.9<<0;if(h!==a.x)h=a.x+(h>a.x?1:-1);f=a.y+f.sin*1.9<<0;if(f!==a.y)f=a.y+(f>a.y?1:-1);for(var i=c.getBodies(h,f,a.width,a.height),j=0;j<i.length;j++){var l=i[j];if(l!==a&&l.intersects(h,f,a.width,a.height))g+=l.intersection(h,f,a.width,a.height)}return g<<0},e=d(b.clone().rotate(-45));d=d(b.clone().rotate(45));if(e<d)this.moveBody(a,b.clone().rotate(-90),1);else e>d&&this.moveBody(a,b.clone().rotate(90),1)};
...
Game.prototype.explodeBomb=function(a,b){if(arguments.length===1)b={x:a.x,y:a.y,bodies:[],fires:[]};var c=a.size*a.width,d=a.size*a.height,e=this;this.removeBody(a);var f=function(g,h,i,j,l){for(g=a.x+g;g<=a.x+i;g+=a.width)for(var k=a.y+h;k<=a.y+j;k+=a.height)if(g>=0&&k>=0&&g<e.world.width&&k<e.world.height)for(var o=e.world.getBodies(g,k,a.width,a.height),n=0;n<o.length;n++){var m=o[n];if(m!==a&&m.intersects(g,k,a.width,a.height)){_.contains(b.bodies,m)||b.bodies.push(m);if(m.armor>=a.power)return;
else{e.removeBody(m);b.fires[g]||(b.fires[g]=[]);b.fires[g][k]||(b.fires[g][k]=l)}m instanceof Bomb&&e.explodeBomb(m,b)}}};f(-c,0,-a.width,0,"l");f(+a.width,0,+c,0,"r");f(0,-d,0,-a.height,"u");f(0,+a.height,0,+d,"d");return b};Game.prototype.addBody=function(a,b){if(!this.world.addBody(a,b))return false;if(a instanceof Player){if(this.players.length==this.maxPlayers)return false;else if(this.players.length===0)this.owner=a;this.players.push(a)}else a instanceof Bomb&&this.bombs.push(a);return true};
Game.prototype.serialize=function(){var a=this,b={width:this.world.width,height:this.world.height},c=function(d,e){if(a[d].length>0){var f=a[d][0].width;b[d]={};b[d].size=f;b[d].pos=[];b[d].attrs={};_.each(a[d],function(g){b[d].pos.push(g.y/f*Math.floor(a.world.width/f)+g.x/f);_.each(e,function(h){b[d].attrs[h]=g[h]})})}};if(this.players.length>0){b.players=[];_.each(this.players,function(d){b.players.push(d.serialize())})}c("blocks",["armor"]);c("bricks");return b};
...
Game.prototype.createBlocks=function(a){for(var b=1;b<this.world.height/a-2;b+=2)for(var c=1;c<this.world.width/a-2;c+=2){var d=new Box(c*a,b*a,a,a);d.armor=2;this.blocks.push(d);this.addBody(d)}return this};Game.prototype.createBricks=function(a,b){for(var c=this.world.width/a,d=this.world.height/a,e=0;e<d-1;e++)for(var f=0;f<c-1;f++)if(Math.random()*100<b&&!(f%2===1&&e%2===1))if((f>1||e>1)&&(f>1||e<d-2)&&(f<c-2||e>1)&&(f<c-2||e<d-2)){var g=new Box(f*a,e*a,a,a);this.bricks.push(g);this.addBody(g)}return this};
Game.deserialize=function(a){var b=new Game(a.width,a.height),c=function(d){if(a[d].pos.length>0){var e=a[d].size,f=Math.floor(a.width/e);_.each(a[d].pos,function(g){var h=new Box(g%f*e,Math.floor(g/f)*e,e,e);_.each(a[d].attrs,function(i,j){h[j]=a[d].attrs[j]});b[d].push(h);b.addBody(h)})}};c("blocks");c("bricks");a.players&&a.players.length>0&&_.each(a.players,function(d){b.addBody(Player.deserialize(d),true)});return b};
