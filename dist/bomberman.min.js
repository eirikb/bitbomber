...
OGE.World.prototype.moveBody=function(a,b,c){var d,f,e;if(arguments.length===1){b=a.direction;c=a.speed}for(var g=0;g<c;g++){d=a.x;f=a.y;this.removeBodyFromZones(a);a.x+=b.cos;a.y+=b.sin;if(a.x>=0&&a.y>=0&&a.x+a.width<=this.width&&a.y+a.height<=this.height){e=this.getBodies(a);for(var h=0;h<e.length;h++){var j=e[h];if(a!==j&&!j.intersects(d,f,a.width,a.height)&&j.intersects(a)){var i=a.collide(j)===true;j=j.collide(a)===true;if(i&&j){a.x=d;a.y=f;this.addBodyToZones(a);if(a.slide&&h>=0)this.slideBody(a,
b);else return}}}this.addBodyToZones(a)}else{a.x=d;a.y=f;break}}};
OGE.World.prototype.slideBody=function(a,b){var c=this,d=function(e){var g=[],h=c.getBodies(a),j,i,k,l,m,o;for(i=0;i<h.length;i++){m=h[i];m!==a&&m.intersects(a)&&g.push(m)}j=0;k=a.x+e.cos*1.9<<0;if(k!==a.x)k=a.x+(k>a.x?1:-1);l=a.y+e.sin*1.9<<0;if(l!==a.y)l=a.y+(l>a.y?1:-1);h=c.getBodies(k,l,a.width,a.height);for(i=0;i<h.length;i++){m=h[i];if(m!==a&&m.intersects(k,l,a.width,a.height)){o=false;for(e=0;e<g.length;e++)if(m===g[e]){o=true;break}o||(j+=m.intersection(k,l,a.width,a.height))}}return j<<0},
...
Game.prototype.explodeBomb=function(a,b){if(arguments.length===1)b={x:a.x,y:a.y,bodies:[],fires:[],bombs:[]};var c=this,d=function(e,g,h){var j=function(i){return i.x===e&&i.y===g};if(typeof _.detect(b.bodies,j)==="undefined"){h={x:e,y:g,firevar:h};j=_.detect(b.fires,j);if(typeof j!=="undefined"){if(j.firevar!=="c"&&_.contains("c","v","h",h.firevar)){b.fires=_.without(b.fires,j);b.fires.push(h)}}else b.fires.push(h)}};b.bombs.push(a);d(a.x,a.y,"c");var f=function(e,g,h,j){e=e*a.width;var i=a.x+e,
k=a.x+a.size*e;g=g*a.height;var l=a.y+g,m=a.y+a.size*g;actualCheck=function(o,p,s){if(o>=0&&p>=0&&o<c.world.width&&p<c.world.height){for(var r=c.world.getBodies(o,p,a.width,a.height),q=0;q<r.length;q++){var n=r[q];if(n!==a&&n.intersects(o+2,p+2,a.width-2,a.height-2)){if(n.armor>a.power)return false;else{!(n instanceof Bomb)&&!_.contains(b.bodies,n)&&b.bodies.push(n);if(n.armor===a.power)return false}n instanceof Bomb&&!_.contains(b.bombs,n)&&c.explodeBomb(n,b)}}d(o,p,s)}return true};for(i=i;i!==k+
e;i+=e)if(!actualCheck(i,a.y,i!==k?h:j))break;for(e=l;e!==m+g;e+=g)if(!actualCheck(a.x,e,e!==m?h:j))break};f(-1,0,"h","l");f(1,0,"h","r");f(0,-1,"v","u");f(0,1,"v","d");return b};Game.prototype.addBody=function(a,b){if(!this.world.addBody(a,b))return false;if(a instanceof Player){if(this.players.length==this.maxPlayers)return false;else if(this.players.length===0)this.owner=a;this.players.push(a)}else a instanceof Bomb&&this.bombs.push(a);return true};
Game.prototype.serialize=function(){var a=this,b={width:this.world.width,height:this.world.height,guid:this.guid},c=function(d,f){if(a[d].length>0){var e=a[d][0].width;b[d]={};b[d].size=e;b[d].pos=[];b[d].attrs={};_.each(a[d],function(g){b[d].pos.push(g.y/e*Math.floor(a.world.width/e)+g.x/e);_.each(f,function(h){b[d].attrs[h]=g[h]})})}};if(this.players.length>0){b.players=[];_.each(this.players,function(d){b.players.push(d.serialize())})}c("blocks",["armor"]);c("bricks");return b};
...
Game.deserialize=function(a){var b=new Game(a.width,a.height),c=function(d){if(a[d].pos.length>0){var f=a[d].size,e=Math.floor(a.width/f);_.each(a[d].pos,function(g){var h=new Box(g%e*f,Math.floor(g/e)*f,f,f);_.each(a[d].attrs,function(j,i){h[i]=a[d].attrs[i]});b[d].push(h);b.addBody(h)})}};c("blocks");c("bricks");a.players&&a.players.length>0&&_.each(a.players,function(d){b.addBody(Player.deserialize(d),true)});b.guid=a.guid;return b};
