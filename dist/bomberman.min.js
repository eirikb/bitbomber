...
OGE.World.prototype.getBodies=function(a,b,d,c){var e,f;e=f=a;if(arguments.length===1){f=e.x;b=e.y;d=e.width;c=e.height}f=f<0?0:f;f=f+d>this.width?this.width-d:f;b=b<0?0:b;b=b+c>this.height?this.height-c:b;e=[];f=this.getZones(f,b,d,c);for(var h=0;h<f.length;h++)for(var k=f[h].bodies,i=0;i<k.length;i++){for(var j=k[i],g=false,l=0;l<e.length;l++)if(e[l]===j){g=true;break}g||e.push(j)}return e};
OGE.World.prototype.getZones=function(a,b,d,c){var e,f;e=f=a;if(arguments.length===1){f=e.x;b=e.y;d=e.width;c=e.height}if(f>=0&&f+d-1<this.width&&b>=0&&b+c-1<this.height){e=(f+d)/this.zoneSize<<0;var h=b/this.zoneSize<<0,k=(b+c)/this.zoneSize<<0,i=0,j=[];for(f=f/this.zoneSize<<0;f<=e;f++)for(b=h;b<=k;b++)j[i++]=this.zones[f][b];return j}else return[]};
...
OGE.World.prototype.moveBody=function(a,b,d){var c,e,f;if(arguments.length===1){b=a.direction;d=a.speed}for(var h=0;h<d;h++){c=a.x;e=a.y;this.removeBodyFromZones(a);a.x+=b.cos;a.y+=b.sin;f=this.getBodies(a);for(var k=0;k<f.length;k++){var i=f[k];if(a!==i&&!i.intersects(c,e,a.width,a.height)&&i.intersects(a)){var j=a.collide(i)===true;i=i.collide(a)===true;if(j&&i){a.x=c;a.y=e;this.addBodyToZones(a);if(a.slide&&k>=0)this.slideBody(a,b);else return}}}this.addBodyToZones(a)}};
OGE.World.prototype.slideBody=function(a,b){var d=this,c=function(f){var h=0,k=a.x+f.cos*1.9<<0;if(k!==a.x)k=a.x+(k>a.x?1:-1);f=a.y+f.sin*1.9<<0;if(f!==a.y)f=a.y+(f>a.y?1:-1);for(var i=d.getBodies(k,f,a.width,a.height),j=0;j<i.length;j++){var g=i[j];if(g!==a&&g.intersects(k,f,a.width,a.height))h+=g.intersection(k,f,a.width,a.height)}return h<<0},e=c(b.clone().rotate(-45));c=c(b.clone().rotate(45));if(e<c)this.moveBody(a,b.clone().rotate(-90),1);else e>c&&this.moveBody(a,b.clone().rotate(90),1)};
...
Game.deserialize=function(a){var b=new Game(a.width,a.height),d=function(c){if(a[c].pos.length>0){var e=a[c].size,f=Math.floor(a.width/e);_.each(a[c].pos,function(h){var k=new Box(h%f*e,Math.floor(h/f)*e,e,e);_.each(a[c].attrs,function(i,j){k[j]=a[c].attrs[j]});b[c].push(k);b.addBody(k)})}};d("blocks");d("bricks");a.players&&a.players.length>0&&_.each(a.players,function(c){b.addBody(Player.deserialize(c),true)});return b};var $infoArea,utils={};
utils.log=function(a,b){if(arguments.length===1)b=a;b.cmd=b.cmd||a;if(b.cmd&&b.result){typeof console!=="undefined"&&console!==null&&console.log(b.cmd,b);b=b.cmd+" - "+b.result}else typeof console!=="undefined"&&console!==null&&console.log(b);$infoArea.val($infoArea.val()+($infoArea.val().length>0?"\n":"")+b);$infoArea.attr("scrollTop",$infoArea.attr("scrollHeight"))};
var bomberman=function(){new LobbyHandler(this);var a=new GameHandler(this);this.startGame=function(b,d){a.startGame(b,d)};this.login=function(b){a.login(b)}};$(function(){$infoArea=$("#infoArea");bomberman()});
LobbyHandler=function(a){var b=new LobbyClient(this);new LobbyPanel(this);var d;this.createGame=function(c){b.createGame(function(e){if(e.result==="OK"){e=Game.deserialize(e.data);a.startGame(e,d.nick);c(true)}else c(fale)})};this.playNow=function(c){b.playNow(function(e){if(e.result==="OK"){e=Game.deserialize(e.data);a.startGame(e,d.nick);c(true)}else c(false)})};this.login=function(c,e){b.login(c,function(f){if(f.result==="OK"){d=f.data;a.login(d.guid);e(true)}else e(false)})}};
LobbyClient=function(){var a;this.createGame=function(b){$.getJSON("/lobby?cmd=createGame&guid="+a.guid,function(d){utils.log("createGame",d);b(d)})};this.playNow=function(){$.getJSON("/lobby?cmd=joinGame&guid="+a.guid,function(b){utils.log("playNow",b)})};this.login=function(b,d){$.getJSON("/lobby?cmd=loginPlayer&nick="+b,function(c){utils.log("loginPlayer",c);if(c.result==="OK")a=c.data;d(c)})}};
LobbyPanel=function(a){var b=$("#loginPanel"),d=$("#loginButton"),c=$("#lobbyPanel"),e=$("#nickField"),f=$("#playNowButton"),h=$("#createGameButton"),k=function(){a.createGame(function(g){g&&c.hide()})},i=function(){a.playNow(function(){c.hide()})},j=function(){e.val().length>0&&a.login(e.val(),function(g){if(g){b.hide();c.show()}else b.children("span").text("Nick taken!")})};(function(){e.keypress(function(g){g.keyCode===13&&j()});d.click(function(){j()});f.click(function(){i()});h.click(function(){k()});
e.focus()})()};
GameHandler=function(){var a=new GamePanel(this),b=new GameClient,d,c;this.startGame=function(e,f){d=e;c=d.getPlayer(f);a.startGame(d)};this.login=function(e){b.send("authPlayer",{guid:e})};this.step=function(){d.world.step()};this.startMove=function(e,f){c.direction=new OGE.Direction(e,f);b.send("startMove",{cos:e,sin:f,x:c.x,y:c.y})};this.endMove=function(){c.direction=null;b.send("endMove",{x:c.x,y:c.y})};this.placeBomb=function(){if(c.bombs>0){c.bombs--;var e=new Bomb(Math.floor((c.x+8)/16)*16,
Math.floor((c.y+8)/16)*16,16,16);e.power=c.power;b.send("placeBomb",{x:e.x,y:e.y});return e}};b.addListener("joinGame",function(){p=Player.deserialize(msg.data.player);d.addBody(p,true);addPlayer(p)});b.addListener("startMove",function(){p=d.getPlayer(msg.data.player);p.direction=new OGE.Direction(msg.data.cos,msg.data.sin);p.x=msg.data.x;p.y=msg.data.y});b.addListener("endMove",function(){p=d.getPlayer(msg.data.player);p.direction=null;p.x=msg.data.x;p.y=msg.data.y});b.addListener("logoutPlayer",
function(){p=d.getPlayer(msg.data.player);p.$img.remove();d.removeBody(p)});b.addListener("placeBomb",function(){var e=new Bomb(msg.data.x,msg.data.y,16,16);placeBomb(e)})};GameClient=function(){var a=new io.Socket,b={};this.addListener=function(d,c){if(typeof b[d]==="undefined")b[d]=[];b[d].push(c)};this.send=function(d,c){a.send({cmd:d,data:c})};a.connect();a.on("message",function(d){utils.log(d);_.each(b[d.cmd],function(c){c(d.result,d.data)})})};
GamePanel=function(a){var b=$("#gamePanel"),d=$("#fpsLabel"),c,e;this.startGame=function(h){c=new KeyboardHandler;e=new FactorialTimer;b.find("*").remove();b.show();b.width(h.world.width).height(h.world.height);$.each(h.blocks,function(i,j){f(j,"block")});$.each(h.bricks,function(i,j){f(j,"brick")});$.each(h.players,function(i,j){f(j,"pl1");j.animate=0;j.sprite=0;j.sprites=[1,2,1,3]});c.keydown(function(i){var j=0,g=0;switch(i){case "space":i=a.placeBomb();i.animate=0;i.sprite=0;i.sprites=[1,2,3];
f(i,"bomb1");break;case "left":j=-1;break;case "up":g=-1;break;case "right":j=1;break;case "down":g=1}if(j!==0||g!==0)a.startMove(j,g)}).keyup(function(){a.endMove()});var k=0;e.start(function(i){a.step();if(++k===20){d.text("Time: "+i);k=0}$.each(h.players,function(j,g){g.$img.css("left",g.x).css("top",g.y-4);if(g.direction!==null&&g.speed>0){if(g.lastDirection!==g.direction){g.animate=0;g.lastDirection=g.direction}if(--g.animate<0){g.animate=5;if(++g.sprite>=g.sprites.length)g.sprite=0;var l;if(g.direction.cos!==
0)l=g.direction.cos>0?"r":"l";else if(g.direction.sin!==0)l=g.direction.sin>0?"d":"u";g.$img.attr("src","/images/p"+l+g.sprites[g.sprite]+".png")}}});$.each(h.bombs,function(j,g){if(--g.animate<0){g.animate=5;if(++g.sprite>=g.sprites.length)g.sprite=0;g.$img.attr("src","/images/bomb"+g.sprites[g.sprite]+".png")}})})};var f=function(h,k){var i=$("<img>").attr("src","images/"+k+".png").css("left",h.x).css("top",h.y).addClass("body");b.append(i);h.$img=i}};
KeyboardHandler=function(){var a,b,d;this.keydown=function(c){b=c;return this};this.keyup=function(c){d=c;return this};$(document).keydown(function(c){var e=null;switch(c.keyCode){case 32:e="space";break;case 37:case 65:e="left";break;case 38:case 87:e="up";break;case 39:case 68:e="right";break;case 40:case 83:e="down"}if(e!==null){if(a!==c.keyCode){a=c.keyCode;b(e)}c.stopPropagation();c.preventDefault();return false}}).keyup(function(c){if(c.keyCode===a){a=0;d()}}).keypress(function(c){switch(c.keyCode){case 32:case 37:case 38:case 39:case 40:c.stopPropagation();
