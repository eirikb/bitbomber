...
OGE.World.prototype.getBodies=function(a,b,c,d){var e,f;e=f=a;if(arguments.length===1){f=e.x;b=e.y;c=e.width;d=e.height}f=f<0?0:f;f=f+c>this.width?this.width-c:f;b=b<0?0:b;b=b+d>this.height?this.height-d:b;e=[];f=this.getZones(f,b,c,d);for(var h=0;h<f.length;h++)for(var g=f[h].bodies,i=0;i<g.length;i++){for(var j=g[i],m=false,l=0;l<e.length;l++)if(e[l]===j){m=true;break}m||e.push(j)}return e};
OGE.World.prototype.getZones=function(a,b,c,d){var e,f;e=f=a;if(arguments.length===1){f=e.x;b=e.y;c=e.width;d=e.height}if(f>=0&&f+c-1<this.width&&b>=0&&b+d-1<this.height){e=(f+c)/this.zoneSize<<0;var h=b/this.zoneSize<<0,g=(b+d)/this.zoneSize<<0,i=0,j=[];for(f=f/this.zoneSize<<0;f<=e;f++)for(b=h;b<=g;b++)j[i++]=this.zones[f][b];return j}else return[]};
...
OGE.World.prototype.moveBody=function(a,b,c){var d,e,f;if(arguments.length===1){b=a.direction;c=a.speed}for(var h=0;h<c;h++){d=a.x;e=a.y;this.removeBodyFromZones(a);a.x+=b.cos;a.y+=b.sin;f=this.getBodies(a);for(var g=0;g<f.length;g++){var i=f[g];if(a!==i&&!i.intersects(d,e,a.width,a.height)&&i.intersects(a)){var j=a.collide(i)===true;i=i.collide(a)===true;if(j&&i){a.x=d;a.y=e;this.addBodyToZones(a);if(a.slide&&g>=0)this.slideBody(a,b);else return}}}this.addBodyToZones(a)}};
OGE.World.prototype.slideBody=function(a,b){var c=this,d=function(f){var h=[],g=c.getBodies(a),i,j,m,l,k,o;for(j=0;j<g.length;j++){k=g[j];k!==a&&k.intersects(a)&&h.push(k)}i=0;m=a.x+f.cos*1.9<<0;if(m!==a.x)m=a.x+(m>a.x?1:-1);l=a.y+f.sin*1.9<<0;if(l!==a.y)l=a.y+(l>a.y?1:-1);g=c.getBodies(m,l,a.width,a.height);for(j=0;j<g.length;j++){k=g[j];if(k!==a&&k.intersects(m,l,a.width,a.height)){o=false;for(f=0;f<h.length;f++)if(k===h[f]){o=true;break}o||(i+=k.intersection(m,l,a.width,a.height))}}return i<<0},
e=d(b.clone().rotate(-45));d=d(b.clone().rotate(45));if(e<d)this.moveBody(a,b.clone().rotate(-90),1);else e>d&&this.moveBody(a,b.clone().rotate(90),1)};Object.construct_prototype=function(a){var b=function(){};b.prototype=a.prototype;return new b};Player=function(a,b,c,d,e){this.nick=e;if(arguments.length===1)this.nick=arguments[0];OGE.Body.apply(this,arguments);this.speed=3;this.slide=true;this.bombPower=this.bombSize=1;this.life=3;this.armor=0;this.power=this.bombs=1};Player.prototype=Object.construct_prototype(OGE.Body);
...
Game.prototype.getBomb=function(a,b){for(var c=0;c<this.bombs.length;c++)if(this.bombs[c].x===a&&this.bombs[c].y===b)return this.bombs[c];return null};Game.prototype.removeBoxes=function(a,b){var c=this;_.each(b.bodies,function(d){d instanceof Box&&d.armor===a.power&&c.removeBody(d)})};
Game.prototype.explodeBomb=function(a,b){if(arguments.length===1)b={x:a.x,y:a.y,bodies:[],fires:[]};var c=a.size*a.width,d=a.size*a.height,e=this;this.removeBody(a);var f=function(g,i,j){b.fires[g]||(b.fires[g]=[]);b.fires[g][i]||(b.fires[g][i]=j)};f(a.x,a.y,"c");var h=function(g,i,j,m,l){for(g=a.x+g;g<=a.x+j;g+=a.width)for(var k=a.y+i;k<=a.y+m;k+=a.height)if(g>=0&&k>=0&&g<e.world.width&&k<e.world.height)for(var o=e.world.getBodies(g,k,a.width,a.height),p=0;p<o.length;p++){var n=o[p];if(n!==a&&n.intersects(g,
k,a.width,a.height)){_.contains(b.bodies,n)||b.bodies.push(n);if(n.armor>=a.power){if(b.fires[g]&&b.fires[g][k])b.fires[g][k]=null;return}else e.removeBody(n);n instanceof Bomb&&e.explodeBomb(n,b)}f(g,k,l)}};h(-c,0,-a.width,0,"l");h(+a.width,0,+c,0,"r");h(0,-d,0,-a.height,"u");h(0,+a.height,0,+d,"d");return b};
...
Game.prototype.createBlocks=function(a){for(var b=1;b<this.world.height/a-2;b+=2)for(var c=1;c<this.world.width/a-2;c+=2){var d=new Box(c*a,b*a,a,a);d.armor=2;this.blocks.push(d);this.addBody(d)}return this};Game.prototype.createBricks=function(a,b){for(var c=this.world.width/a,d=this.world.height/a,e=0;e<d-1;e++)for(var f=0;f<c-1;f++)if(Math.random()*100<b&&!(f%2===1&&e%2===1))if((f>1||e>1)&&(f>1||e<d-2)&&(f<c-2||e>1)&&(f<c-2||e<d-2)){var h=new Box(f*a,e*a,a,a);this.bricks.push(h);this.addBody(h)}return this};
Game.deserialize=function(a){var b=new Game(a.width,a.height),c=function(d){if(a[d].pos.length>0){var e=a[d].size,f=Math.floor(a.width/e);_.each(a[d].pos,function(h){var g=new Box(h%f*e,Math.floor(h/f)*e,e,e);_.each(a[d].attrs,function(i,j){g[j]=a[d].attrs[j]});b[d].push(g);b.addBody(g)})}};c("blocks");c("bricks");a.players&&a.players.length>0&&_.each(a.players,function(d){b.addBody(Player.deserialize(d),true)});return b};
