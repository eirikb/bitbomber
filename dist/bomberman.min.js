...
OGE.World.prototype.getBodies=function(a,b,c,d){var f,e;f=e=a;if(arguments.length===1){e=f.x;b=f.y;c=f.width;d=f.height}e=e<0?0:e;e=e+c>this.width?this.width-c:e;b=b<0?0:b;b=b+d>this.height?this.height-d:b;f=[];e=this.getZones(e,b,c,d);for(var g=0;g<e.length;g++)for(var i=e[g].bodies,j=0;j<i.length;j++){for(var k=i[j],o=false,h=0;h<f.length;h++)if(f[h]===k){o=true;break}o||f.push(k)}return f};
OGE.World.prototype.getZones=function(a,b,c,d){var f,e;f=e=a;if(arguments.length===1){e=f.x;b=f.y;c=f.width;d=f.height}if(e>=0&&e+c-1<this.width&&b>=0&&b+d-1<this.height){f=(e+c)/this.zoneSize<<0;var g=b/this.zoneSize<<0,i=(b+d)/this.zoneSize<<0,j=0,k=[];for(e=e/this.zoneSize<<0;e<=f;e++)for(b=g;b<=i;b++)k[j++]=this.zones[e][b];return k}else return[]};
...
OGE.World.prototype.moveBody=function(a,b,c){var d,f,e;if(arguments.length===1){b=a.direction;c=a.speed}for(var g=0;g<c;g++){d=a.x;f=a.y;this.removeBodyFromZones(a);a.x+=b.cos;a.y+=b.sin;if(a.x>=0&&a.y>=0&&a.x+a.width<this.width&&a.y+a.height<this.height){e=this.getBodies(a);for(var i=0;i<e.length;i++){var j=e[i];if(a!==j&&!j.intersects(d,f,a.width,a.height)&&j.intersects(a)){var k=a.collide(j)===true;j=j.collide(a)===true;if(k&&j){a.x=d;a.y=f;this.addBodyToZones(a);if(a.slide&&i>=0)this.slideBody(a,
b);else return}}}this.addBodyToZones(a)}else{a.x=d;a.y=f;break}}};
OGE.World.prototype.slideBody=function(a,b){var c=this,d=function(e){var g=[],i=c.getBodies(a),j,k,o,h,q,m;for(k=0;k<i.length;k++){q=i[k];q!==a&&q.intersects(a)&&g.push(q)}j=0;o=a.x+e.cos*1.9<<0;if(o!==a.x)o=a.x+(o>a.x?1:-1);h=a.y+e.sin*1.9<<0;if(h!==a.y)h=a.y+(h>a.y?1:-1);i=c.getBodies(o,h,a.width,a.height);for(k=0;k<i.length;k++){q=i[k];if(q!==a&&q.intersects(o,h,a.width,a.height)){m=false;for(e=0;e<g.length;e++)if(q===g[e]){m=true;break}m||(j+=q.intersection(o,h,a.width,a.height))}}return j<<0},
...
Game.prototype.explodeBomb=function(a,b){if(arguments.length===1)b={x:a.x,y:a.y,bodies:[],fires:[],bombs:[]};var c=this,d=function(e,g,i){var j=function(k){return k.x===e&&k.y===g};if(typeof _.detect(b.bodies,j)==="undefined"){i={x:e,y:g,firevar:i};j=_.detect(b.fires,j);if(typeof j!=="undefined"){if(j.firevar!=="c"&&_.contains("c","v","h",i.firevar)){b.fires=_.without(b.fires,j);b.fires.push(i)}}else b.fires.push(i)}};b.bombs.push(a);d(a.x,a.y,"c");var f=function(e,g,i,j){e=e*a.width;var k=a.x+e,
o=a.x+a.size*e;g=g*a.height;var h=a.y+g,q=a.y+a.size*g;actualCheck=function(m,r,n){if(m>=0&&r>=0&&m<c.world.width&&r<c.world.height){for(var s=c.world.getBodies(m,r,a.width,a.height),l=0;l<s.length;l++){var t=s[l];if(t!==a&&t.intersects(m+2,r+2,a.width-2,a.height-2)){if(t.armor>a.power)return false;else{!(t instanceof Bomb)&&!_.contains(b.bodies,t)&&b.bodies.push(t);if(t.armor===a.power)return false}t instanceof Bomb&&!_.contains(b.bombs,t)&&c.explodeBomb(t,b)}}d(m,r,n)}return true};for(k=k;k!==o+
e;k+=e)if(!actualCheck(k,a.y,k!==o?i:j))break;for(e=h;e!==q+g;e+=g)if(!actualCheck(a.x,e,e!==q?i:j))break};f(-1,0,"h","l");f(1,0,"h","r");f(0,-1,"v","u");f(0,1,"v","d");return b};Game.prototype.addBody=function(a,b){if(!this.world.addBody(a,b))return false;if(a instanceof Player){if(this.players.length==this.maxPlayers)return false;else if(this.players.length===0)this.owner=a;this.players.push(a)}else a instanceof Bomb&&this.bombs.push(a);return true};
Game.prototype.serialize=function(){var a=this,b={width:this.world.width,height:this.world.height},c=function(d,f){if(a[d].length>0){var e=a[d][0].width;b[d]={};b[d].size=e;b[d].pos=[];b[d].attrs={};_.each(a[d],function(g){b[d].pos.push(g.y/e*Math.floor(a.world.width/e)+g.x/e);_.each(f,function(i){b[d].attrs[i]=g[i]})})}};if(this.players.length>0){b.players=[];_.each(this.players,function(d){b.players.push(d.serialize())})}c("blocks",["armor"]);c("bricks");return b};
...
Game.prototype.createBlocks=function(a){for(var b=1;b<this.world.height/a-2;b+=2)for(var c=1;c<this.world.width/a-2;c+=2){var d=new Box(c*a,b*a,a,a);d.armor=2;this.blocks.push(d);this.addBody(d)}return this};Game.prototype.createBricks=function(a,b){for(var c=this.world.width/a,d=this.world.height/a,f=0;f<d-1;f++)for(var e=0;e<c-1;e++)if(Math.random()*100<b&&!(e%2===1&&f%2===1))if((e>1||f>1)&&(e>1||f<d-2)&&(e<c-2||f>1)&&(e<c-2||f<d-2)){var g=new Box(e*a,f*a,a,a);this.bricks.push(g);this.addBody(g)}return this};
Game.deserialize=function(a){var b=new Game(a.width,a.height),c=function(d){if(a[d].pos.length>0){var f=a[d].size,e=Math.floor(a.width/f);_.each(a[d].pos,function(g){var i=new Box(g%e*f,Math.floor(g/e)*f,f,f);_.each(a[d].attrs,function(j,k){i[k]=a[d].attrs[k]});b[d].push(i);b.addBody(i)})}};c("blocks");c("bricks");a.players&&a.players.length>0&&_.each(a.players,function(d){b.addBody(Player.deserialize(d),true)});return b};var $infoArea,utils={};
...
$(function(){$infoArea=$("#infoArea");var a=new HttpClient,b=new SocketClient,c=new GameHandler(d,b),d=new LobbyHandler(c,a,b);new GamePanel(c);new LobbyPanel(d);utils.log({cmd:"versions",result:"OGE: "+OGE.version+". Game: "+Game.version+". Client: 0.1"})});
LobbyHandler=function(a,b,c){b=new HttpClient(this);var d;this.createGame=function(f){b.createGame(function(e){if(e.result==="OK"){e=Game.deserialize(e.data);a.startGame(e,d.nick);f(true)}else f(fale)})};this.playNow=function(f){b.playNow(function(e){if(e.result==="OK"){e=Game.deserialize(e.data);a.startGame(e,d.nick);f(true)}else f(false)})};this.login=function(f,e){b.login(f,function(g){if(g.result==="OK"){d=g.data;c.send("authPlayer",{guid:d.guid});e(true)}else e(false)})}};
...
LobbyPanel=function(a){var b=$("#loginPanel"),c=$("#loginButton"),d=$("#lobbyPanel"),f=$("#nickField"),e=$("#playNowButton"),g=$("#createGameButton"),i=function(){a.createGame(function(o){o&&d.hide()})},j=function(){a.playNow(function(){d.hide()})},k=function(){f.val().length>0&&a.login(f.val(),function(o){if(o){b.hide();d.show()}else{f.focus();b.children("span").text("Nick taken!")}})};(function(){f.keypress(function(o){if(o.keyCode===13){f.blur();k()}});c.click(function(){k()});e.click(function(){j()});
g.click(function(){i()});f.focus()})()};
GameHandler=function(a,b){var c,d,f;listeners={};this.addListener=function(e,g){if(typeof listeners[e]==="undefined")listeners[e]=[];listeners[e].push(g)};this.removeBody=function(e){c.removeBody(e)};this.startGame=function(e,g){c=e;d=c.getPlayer(g);f=new FactorialTimer;var i=(new Date).getTime(),j;f.start(function(){i=(new Date).getTime()-i;j=(new Date).getTime();c.world.step();j=(new Date).getTime()-j;var k=Math.floor(1E3/i);_.each(listeners.step,function(o){o(i,k,j)});i=(new Date).getTime()});
_.each(listeners.startGame,function(k){k(c)})};this.startMove=function(e,g){if(!d.dead){d.direction=new OGE.Direction(e,g);b.send("startMove",{cos:e,sin:g,x:d.x,y:d.y})}};this.endMove=function(){if(!d.dead){d.direction=null;b.send("endMove",{x:d.x,y:d.y})}};this.placeBomb=function(){if(!d.dead)if(d.bombs>0){var e=new Bomb(Math.floor((d.x+8)/16)*16,Math.floor((d.y+8)/16)*16,16,16);e.size=2;c.addBody(e);e.power=d.power;b.send("placeBomb",{x:e.x,y:e.y});return e}};b.addListener("joinGame",function(e,
g){p=Player.deserialize(g.player);c.addBody(p,true);_.each(listeners.addPlayer,function(i){i(p)})});b.addListener("startMove",function(e,g){p=c.getPlayer(g.player);p.direction=new OGE.Direction(g.cos,g.sin);p.x=g.x;p.y=g.y});b.addListener("endMove",function(e,g){p=c.getPlayer(g.player);p.direction=null;p.x=g.x;p.y=g.y});b.addListener("logoutPlayer",function(e,g){p=c.getPlayer(g.player);p.$img.remove();c.removeBody(p)});b.addListener("placeBomb",function(e,g){var i=new Bomb(g.x,g.y,16,16);c.addBody(i);
_.each(listeners.placeBomb,function(j){j(i)})});b.addListener("explodeBomb",function(e,g){var i=c.getBomb(g.x,g.y);c.getPlayer(g.player).bombs++;if(i!==null){g=c.explodeBomb(i);c.removeBodies(g.bombs,Bomb);if(_.include(g.bodies,d)){_.each(listeners.meDead,function(){});d.x=0;d.y=0;b.send("playerDead",{})}_.each(listeners.explodeBomb,function(j){j(i,g)})}});b.addListener("playerDead",function(e,g){var i=c.getPlayer(g.player);i.x=0;i.y=0;_.each(listeners.playerDead,function(){})});b.addListener("resurectPlayer",
function(e,g){var i=c.getPlayer(g.player);c.addBody(i);_.each(listeners.resurectPlayer,function(j){j(i)})})};SocketClient=function(){var a=new io.Socket,b={};this.addListener=function(c,d){if(typeof b[c]==="undefined")b[c]=[];b[c].push(d)};this.send=function(c,d){a.send({cmd:c,data:d})};a.connect();a.on("message",function(c){utils.log(c);_.each(b[c.cmd],function(d){d(c.result,c.data)})})};
GamePanel=function(a){var b=$("#gamePanel"),c=$("#fpsLabel"),d,f=[],e=[];a.addListener("startGame",function(h){d=new KeyboardHandler;b.find("*").remove();b.show();b.width(h.world.width).height(h.world.height);_.each(h.blocks,function(m){g(m,"objects")});_.each(h.bricks,function(m){g(m,"objects");m.offsetSprite=1;o(m)});_.each(h.players,function(m){i(m)});d.keydown(function(m){var r=0,n=0;switch(m){case "space":(m=a.placeBomb())&&j(m);break;case "left":r=-1;break;case "up":n=-1;break;case "right":r=
1;break;case "down":n=1}if(r!==0||n!==0)a.startMove(r,n)}).keyup(function(){a.endMove()});var q=0;a.addListener("step",function(m,r,n){var s=(new Date).getTime();_.each(h.players,function(l){if(l.animate===0&&l.direction!==null&&l.speed>0){l.animate=5;l.sprite=0}else if(l.animate>0&&l.direction===null||l.speed===0)l.animate=0;k(l)});_.each(h.bombs,function(l){k(l)});_.each(f,function(l){k(l);if(l.sprite===l.sprites.length-1&&l.animate===1){f=_.without(f,l);l.$img.remove()}});_.each(e,function(l){k(l);
if(l.sprite===l.sprites.length-1&&l.animate===1){e=_.without(e,l);l.$img.remove();a.removeBody(l)}});if(++q===20){s=(new Date).getTime()-s;c.text("Total time: "+m+". Engine time: "+n+". Drawing time: "+s+" . fps: "+r);q=0}});a.addListener("explodeBomb",function(m,r){_.each(r.bombs,function(n){n.$img.remove()});_.each(r.fires,function(n){var s=0;switch(n.firevar){case "l":s=2;break;case "r":s=3;break;case "u":s=4;break;case "d":s=1;break;case "h":s=5;break;case "v":s=6}g(n,"fires");n.direction=null;
n.sprites=[0,1,2,3];n.offsetSprite=s;n.animate=5;f.push(n)});_.each(r.bodies,function(n){if(n instanceof Box){n.animate=5;n.sprite=0;n.offsetSprite=1;n.sprites=[1,2];n.animateTimer=10;e.push(n)}else n instanceof Bomb&&n.$img.remove()})})});a.addListener("addPlayer",function(h){i(h)});a.addListener("placeBomb",function(h){j(h)});a.addListener("meDead",function(h){h.$img.remove()});a.addListener("playerDead",function(h){h.$img.remove()});a.addListener("resurectPlayer",function(h){b.append(h.$img)});
var g=function(h,q){var m=$("<div>").css("background","url(images/"+q+".png)").css("left",h.x).css("top",h.y).addClass("body");h.$img=m;h.sprite=0;h.offsetSprite=0;h.animate=0;h.animateCount=0;h.sprites=[0];h.animateTimer=5;o(h);b.append(m);return m},i=function(h){g(h,"pb").addClass("player");h.sprites=[0,1,0,2]},j=function(h){g(h,"objects");h.sprites=[0,1,2];h.animate=5;h.offsetSprite=2},k=function(h){h.direction!==null&&h.speed>0&&h.$img.css("left",h.x).css("top",h.y-4);if(h.animate>0){if(h.lastDirection!==
h.direction){h.animate=0;h.lastDirection=h.direction}if(--h.animate<=0){h.animate=h.animateTimer;if(++h.sprite>=h.sprites.length)h.sprite=0;var q=0;if(h.direction!==null)if(h.direction.cos!==0)q=h.direction.cos>0?3:2;else if(h.direction.sin!==0)q=h.direction.sin>0?0:1;o(h,q)}}},o=function(h,q){if(arguments.length===1)q=0;h.$img.css("background-position",-(18*(q+h.offsetSprite))+"px "+-(22*h.sprites[h.sprite])+"px")}};
