...
OGE.World.prototype.slideBody=function(a,b){var c=this,d=function(f){var h=0,g=a.x+f.cos*1.9<<0;if(g!==a.x)g=a.x+(g>a.x?1:-1);f=a.y+f.sin*1.9<<0;if(f!==a.y)f=a.y+(f>a.y?1:-1);for(var i=c.getBodies(g,f,a.width,a.height),j=0;j<i.length;j++){var l=i[j];if(l!==a&&l.intersects(g,f,a.width,a.height))h+=l.intersection(g,f,a.width,a.height)}return h<<0},e=d(b.clone().rotate(-45));d=d(b.clone().rotate(45));if(e<d)this.moveBody(a,b.clone().rotate(-90),1);else e>d&&this.moveBody(a,b.clone().rotate(90),1)};
...
Game.prototype.explodeBomb=function(a,b){if(arguments.length===1)b={x:a.x,y:a.y,bodies:[],fires:[]};var c=a.size*a.width,d=a.size*a.height,e=this;this.removeBody(a);var f=function(g,i,j){b.fires[g]||(b.fires[g]=[]);b.fires[g][i]||(b.fires[g][i]=j)};f(a.x,a.y,"c");var h=function(g,i,j,l,n){for(g=a.x+g;g<=a.x+j;g+=a.width)for(var k=a.y+i;k<=a.y+l;k+=a.height)if(g>=0&&k>=0&&g<e.world.width&&k<e.world.height)for(var p=e.world.getBodies(g,k,a.width,a.height),o=0;o<p.length;o++){var m=p[o];if(m!==a&&m.intersects(g,
k,a.width,a.height)){_.contains(b.bodies,m)||b.bodies.push(m);if(m.armor>=a.power){if(b.fires[g]&&b.fires[g][k])b.fires[g][k]=null;return}else e.removeBody(m);m instanceof Bomb&&e.explodeBomb(m,b)}f(g,k,n)}};h(-c,0,-a.width,0,"l");h(+a.width,0,+c,0,"r");h(0,-d,0,-a.height,"u");h(0,+a.height,0,+d,"d");return b};
